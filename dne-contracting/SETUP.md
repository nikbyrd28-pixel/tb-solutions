...
## Sketch flow (updated)
No prices on the public site. Flow: consultation form → straight into the sketch (`#sketch?type=…`, preset to the room they picked; "plumbing repair" opens a bathroom) with a green "Received — got it, {first name}, expect a text/call/email within one business day" banner at the top and a "Skip this for now" link to the thank-you page. If they came the other way (sketch first, then the form), the sketch is attached to the form and they land on the thank-you page instead. The sketch has floor plan + 3D room (three.js, vendored in `vendor/three/`, loaded only when 3D is opened), wall types (full, half, glass partition, closet), an existing/repair/replace/new status on every item, and a live estimate for what was drawn.
